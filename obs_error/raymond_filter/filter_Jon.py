...
def main(argv=None):
    ...
    parser.add_option("--epsilon",      dest="epsilon", type="float",  default=-1., help='Test different epsilons instead of cutoff')
    parser.add_option("--wrf", action='store_true',help = "WRF Output Only")
    ...
    if options.epsilon > 0:
       eps = options.epsilon
       file_new = file + "_%depsilon" % (options.epsilon)
    else:
    ...
    for item in variable_list:
      ...
      for ti in range(0, len(ncfile.dimensions['time'])):
        ...
        print(ncfile.variables[item].dimensions[:])
           ...
           #--- JDL juding if it is a field that can be shrunk
        ...
        #--- END WRF SPECFIC
        else:  #--- BEGIN CM1 SPECIFIC
           #if not ((ncfile.variables[item].dimensions[-1] == 'west_east_stag' or ncfile.variables[item].dimensions[-1] == 'west_east') and
           #    (ncfile.variables[item].dimensions[-2] == 'south_north_stag' or ncfile.variables[item].dimensions[-2] == 'south_north')):
         if ncfile.variables[item].ndim > 1:
           if not ((ncfile.variables[item].dimensions[-1] == 'xh' or ncfile.variables[item].dimensions[-1] == 'xf' or ncfile.variables[item].dimensions[-1] == 'ni' or ncfile.variables[item].dimensions[-1] == 'nip1') and
               (ncfile.variables[item].dimensions[-2] == 'yh' or ncfile.variables[item].dimensions[-2] == 'yf' or ncfile.variables[item].dimensions[-2] == 'nj' or ncfile.variables[item].dimensions[-2] == 'njp1') and
               (ncfile.variables[item].dimensions[0] == 'time')):
             ...
             print('JDL Vert Dims = ',farray.shape[0])
             for k in range(0, farray.shape[0]): 
                 filtered = raymond_lowpass.raymond2d_lowpass(farray[k].transpose(),eps)
                 if (k==0): print(ti, ncfile2.variables[item][0,k,:,:].shape, filtered.shape)
                 ncfile2.variables[item][ti,k] = filtered.transpose()
